...
class _Permutation(Flow):
    ...
    def __init__(self, permutation, dim=2):
    ...
    def _permute(inputs, permutation, dim):
        ...
        n_poisson = inputs.shape[1]
        outputs = torch.index_select(inputs, dim, permutation)
        logabsdet = torch.zeros(batch_size,n_poisson)
...
class _Linear(Flow):
    ...
    def forward(self, inputs, context=None):
        if not self.training and self.using_cache:
            ...
            logabsdet = self.cache.logabsdet * torch.ones(outputs.shape[0],outputs.shape[1])
            return outputs, logabsdet
        else:
...
class _LULinear(_Linear):
    ...
    def forward_no_cache(self, inputs):
        ...
        logabsdet = self.logabsdet() * inputs.new_ones(outputs.shape[0],outputs.shape[1])
        return outputs, logabsdet
    ...
    def inverse_no_cache(self, inputs):
        ...
        outputs = torch.linalg.solve_triangular(
            lower, torch.transpose(outputs,1,2), upper=False, unitriangular=True
        )
        outputs = torch.linalg.solve_triangular(
        ...
        logabsdet = logabsdet * inputs.new_ones(outputs.shape[0],outputs.shape[1])
