...
class ClassCondFlow(nn.Module):
    ...
    def __init__(self, q0, flows, net):
        """Constructor
...
#             log_q_bis= torch.zeros(len(z), dtype=x.dtype, device=x.device)
        ...
        log_q += log_p0.view(batch_size, num_samples)
        del(log_p0,z,context)
...
class ClassCondFlow_backup(nn.Module):
    """
    Class conditional normalizing Flow model
    """
    ...
    def forward_kld(self, x, y):
        ...
        for i in range(len(self.flows) - 1, -1, -1):
            z, log_det = self.flows[i].inverse(z)
            log_q_bis += log_det
        log_q_bis = log_q_bis.view(-1, num_samples, *log_q_bis.size()[1:])
        log_q += log_q_bis
